...
const HeroContent = ({ filterFunctions }) => {
  ...
  const applyAiFilters = (filters) => {
    ...
    if (filters.bedrooms) filterFunctions.handlebedrooms(filters.bedrooms);
    if (filters.bathrooms) filterFunctions.handlebathroms(filters.bathrooms);
    ...
    if (filters.propertyType) {
      ...
      if (mapped) filterFunctions.setPropertyTypes([mapped]);
    }
    ...
    if (locationText) filterFunctions.setSearchQuery(locationText);
    ...
    if (parts.length > 0) setAiHint(`AI understood: ${parts.join(", ")}`);
  };
